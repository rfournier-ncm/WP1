...
acc_cond <- acc_pid %>%
  group_by(cond) %>%
  summarise(average_cond = mean(perc),
            sd_cond = sd(perc))
```
...
acc_global <- acc_pid %>%
  ...
  summarise(average_perc = mean(perc),
            sd_perc = sd(perc))
```
...
Accuracy as factor for plotting
```{r}
data <- data %>%
  mutate(acc = as.factor(as.numeric(acc)))
```
...
ggsave("/Users/rfournier/Rstudio/pilot5_dmc/figures_exp_1/rt_line_group.jpeg",
        width = 8, height = 6)
```
...
p4.6
 ...
 ggsave("/Users/rfournier/Rstudio/pilot5_dmc/figures_exp_1/acc_line_group.jpeg",
        width = 8, height = 6)
```
...
ggsave("/Users/rfournier/Rstudio/pilot5_dmc/figures_exp_1/rt_acc_line.jpeg",
        width = 15, height = 6)
```
...
```{r}
write_csv(data, "/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/data/data.csv")
data<-read_csv("/Users/rfournier/Rstudio/EMC_WP1_1_git/EMC_WP1_1/data/data.csv")
```
