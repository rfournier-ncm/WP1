...
```{r load-pkg}
pkg <- c("tidyverse", "RColorBrewer", "patchwork", "gtsummary")
...
p.acc
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/acc_pid_barplot.jpeg",
       width = 8, height = 6)
```
...
p.acc_without <-ggplot(acc_pid_check_without, aes(x=pid, y=perc)) +
   ...
   ylab("Accuracy [%]") + ylim(c(0,100))
p.acc_without
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/acc_pid_barplot_without.jpeg",
       width = 8, height = 6)
```

##saving data
```{r}
write_csv(data, "data/data_exp_2/data_WP1_2_model.csv")
data<-read_csv("data/data_exp_2/data_WP1_2_model.csv")
```
...
ggsave("figures/rt_line_pid.jpeg",
      width = 8, height = 8)
```
...
ggsave("figures/acc_line_pid_prob.jpeg",
       width = 8, height = 8)
```
...
ggsave("figures/rt_line_group.jpeg",
        width = 8, height = 6)
```
...
ggsave("figures/acc_line_group.jpeg",
        width = 8, height = 6)
```
...
ggsave("figures/rt_acc_line_group.jpeg",
        width = 15, height = 6)
```
...
ggsave("figures/acc_barplot_group.jpeg",
        width = 6, height = 8)
```
...
ggsave("figures/rt_barplot_group.jpeg",
        width = 6, height = 8)
```
