...
## load packages ##
```{r load-pkg}
pkxg <- c("EMC2", "tidyverse", "RColorBrewer", "patchwork", "tidybayes", "DataExplorer","ggpubr")
...
load("models/LBA_exp2_v2_bias.RData")
```

#Quick plots
```{r}
#Bias model
plot_pars(LBA_exp2_v2_bias,selection="mu",layout=c(2,5), map = TRUE)
posterior_summary(LBA_exp2_v2_bias,selection="alpha",probs = c(0.025, 0.5, 0.975),map = TRUE)#for each participant
posterior_summary(LBA_exp2_v2_bias,selection="mu",probs = c(0.025, 0.5, 0.975),map = TRUE)#at group level
```
...
#Bias model
param_2_pid = EMC2::parameters(LBA_exp2_v2_bias, selection = "alpha", map = TRUE)
```
...
array_theta_pid_2 <- array_theta_2 %>%
  pivot_longer(cols = c("v_SNew_lMTRUE", "v_SNew_lMFALSE", "v_SFamous_lMTRUE", "v_SFamous_lMFALSE", "sv_lMTRUE", "B_lRNew", "B_lRFamous", "A_lRNew", "A_lRFamous", "t0"), names_to = "param", values_to = "value")%>%
  add_column(datapoint = rep(1:30165,each=10,times=30))%>%
  rename(pid=subjects)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/all_params_bias_2.jpeg",
       width = 8, height = 6)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/all_params_pid_bias_2.jpeg",
       width = 8, height = 6)
```
...
thresh_pid_2 <- array_theta_pid_2 %>% 
  ...
  mutate(response = if_else(str_detect(param, "lRNew"), "New", "Famous"),
...
## save a figure
 ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/thresh_bias.jpeg",width = 10, height = 6)
```
...
# save a figure
 ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/thresh2_bias.jpeg",
        width = 21, height = 10)
```
...
# save a figure
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/thresh2_2_bias.jpeg",
       width = 15, height = 10)
```
...
# save a figure
ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/thresh2_3.jpeg",
       width = 21, height = 10)
```
...
drift_rate_pid_2 <- array_theta_pid_2 %>% 
  ...
  mutate(condition = if_else(str_detect(param, "New"), "New", "Famous"),
...
ggsave("figures/drift_rate_2_bias.jpeg",
        width = 10, height = 6)
```
...
```{r}
p4.1 / p4.2
ggsave("figures/drift_rate_true_false_2_bias.jpeg", 
       width = 16, height = 13)
```
...
p4.4 <- ggplot(drift_acc_cond_diff_2, aes(x = diff, y = "")) +
  ...
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Group level:Drift rate (unf > fam)",
       x="Drift rate", y="") +
  xlim(-1,5)
p4.4
```
...
# save a figure
ggsave("figures/drift_rat_mini_2_bias.jpeg",
       width = 15, height = 10)
```
...
# save a figure
ggsave("figures/drift_rate_pid_2_bias.jpeg",
       width = 15, height = 10)
```
...
bias_group <- theta_group_2 %>% 
  filter(str_detect(param, "A_")) %>% 
  mutate(response = if_else(str_detect(param, "lRFamous"), "Famous", "New"),
         response = factor(response,
                            levels = c("Famous", "New")))
head(bias_group)
str(bias_group)
...
## save a figure
 ggsave("figures/bias_2_bias.jpeg",width = 10, height = 6)
```
...
p3.3 <- ggplot(bias_diff_pid, aes(x = diff, y = pid,
  ...
  stat_halfeye(alpha = 0.7) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Participant level: Bias (unf_key > fam_key)", 
...
p3.4 <- ggplot(bias_diff, aes(x = diff, y=pid)) +  
  stat_halfeye(data = bias_diff_pid,
    ...
    stat_halfeye(alpha = 0.9, fill="firebrick",
               position = position_nudge(y=0)) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Bias (unf_key > fam_key)", 
...
# save a figure
 ggsave("figures/bias_2_2_bias.jpeg",
        width = 21, height = 10)
```
