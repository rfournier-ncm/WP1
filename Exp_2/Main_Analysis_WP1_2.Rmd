...
```{r}
#Preregistered model
param_1_pid = EMC2::parameters(LBA_exp2_v1,selection="alpha",map = TRUE)
...
array_theta_pid_1 <- array_theta_1 %>%
  pivot_longer(cols = c("v_SNew_lMTRUE", "v_SNew_lMFALSE", "v_SFamous_lMFALSE", "v_SFamous_lMTRUE", "sv_lMTRUE", "B_lRNew", "B_lRFamous", "A", "t0"), names_to = "param", values_to = "value")%>%
...
ggsave("figures/all_params_1_pilot.jpeg",
       width = 8, height = 6)
```
...
ggsave("figures/all_params_pid_1.jpeg",
       width = 8, height = 6)
```
...
## save a figure
 ggsave("figures/thresh.jpeg",width = 10, height = 6)
```
...
p3.3 <- ggplot(thresh_diff_pid_1, aes(x = diff, y = pid,
  ...
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Participant level: Threshold (Famous > New)", 
...
# save a figure
 ggsave("figures/thresh_pilot.jpeg",
        width = 21, height = 10)
```
