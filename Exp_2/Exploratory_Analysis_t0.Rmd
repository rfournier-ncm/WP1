...
```{r}
#Preregistered model
load("models/LBA_exp2_v3_t0.RData")
```
...
```{r}
#Preregistered model
param_1_pid = EMC2::parameters(LBA_exp2_v3_t0,selection="alpha",map = TRUE)
...
array_theta_pid_1 <- array_theta_1 %>%
  pivot_longer(cols = c("v_SNew_lMTRUE", "v_SNew_lMFALSE", "v_SFamous_lMTRUE", "v_SFamous_lMFALSE", "sv_lMTRUE", "B_lRNew", "B_lRFamous", "A", "t0_SNew", "t0_SFamous"), names_to = "param", values_to = "value")%>%
  add_column(datapoint = rep(1:30501,each=10,times=30))%>% #here 30'000 = number of chains (3) x number of iteration (10'000)
  rename(pid=subjects)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/all_params_1_t0.jpeg",
       width = 8, height = 6)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/all_params_pid_1_t0.jpeg",
       width = 8, height = 6)
```
...
## save a figure
 ggsave("/Users/rfournier/Rstudio/EMC_WP1_2_git/EMC_WP1_2/figures/thresh_t0.jpeg",width = 10, height = 6)
```
...
# save a figure
ggsave("figures/thresh_2_t0.jpeg",
       width = 15, height = 10)
```
...
# save a figure
ggsave("figures/thresh_3_t0.jpeg",
       width = 21, height = 10)
```
...
ggsave("figures/drift_rate_1_t0.jpeg",
        width = 10, height = 6)
```
...
```{r}
p4.1 / p4.2
ggsave("figures/drift_rate_true_false_1_t0.jpeg", 
       width = 16, height = 13)
```
...
# save a figure
ggsave("figures/drift_rat_mini_1_t0.jpeg",
       width = 15, height = 10)
```
...
t0_pid_1 <- array_theta_pid_1 %>% 
  ...
  mutate(response = if_else(str_detect(param, "SFamous"), "Famous", "New"),
         response = factor(response,
                            levels = c("Famous", "New")))
head(t0_pid_1)
str(t0_pid_1)
...
p3.3 <- ggplot(t0_diff_pid_1, aes(x = diff, y = pid,
  ...
  stat_halfeye(alpha = 0.7) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Participant level: Non-decision time (Famous > New)", 
